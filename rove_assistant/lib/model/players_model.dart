...
class PlayersModel extends ChangeNotifier {
  ...
  static const _infectedClass = 'Infected';
  ...
  addPlayer({required RoverClass roverClass, required String playerName}) {
    ...
    Analytics.logPlayerEvent(player, 'add_player');
  ...
  void reactivate(Player player) {
    ...
    Analytics.logPlayerEvent(player, 'reactivate');
    _campaign.reactivatePlayer(player);
    _onStateChanged();
  }

  void deactivate(Player player) {
    Analytics.logPlayerEvent(player, 'deactivate');
  ...
  void addTrait({required Player player, required String trait}) {
    ...
    Analytics.logPlayerEvent(player, 'add_trait', parameters: {
      'trait': trait,
    });
    player.addTrait(trait);
    if (infectedTraits.any((t) => t.name == trait)) {
      player.xulcTrait = trait;
    }
    _onStateChanged();
  }

  void removeTrait({required Player player, required String trait}) {
    Analytics.logPlayerEvent(player, 'remove_trait', parameters: {
      'trait': trait,
    });
  ...
  List<TraitDef> get infectedTraits =>
      _campaignDefinition.getClass(className: _infectedClass).traits;
  ...
  void setResignXulcHealthIncrease(
    ...
    Analytics.logPlayerEvent(player, 'resign_xulc_health_increase');
  ...
  void resetEvolutionOfPlayer(Player player) {
    Analytics.logPlayerEvent(player, 'reset_evolution');
