...
class CampaignModel extends ChangeNotifier {
  ...
  Campaign? get campaignOrNull => __campaign;
  ...
  load(CampaignDef definition, CampaignPersistor persistor) {
    ...
    final latestCampaign = persistor.currentCampaign;
    __campaign = latestCampaign;
  ...
  void newCampaign(String name,
    ...
    Analytics.logEvent('new_campaign', parameters: {
      'include_xulc': includeXulc.toString(),
      'skip_core': skipCore.toString()
    });
  ...
  void deleteCampaign(Campaign campaign) {
    Analytics.logEvent('delete_campaign');
  ...
  void importCampaign(Campaign campaign) {
    Analytics.logEvent('import_campaign');
  ...
  void completeEncounter(
    ...
    for (Player player in _players) {
      ...
      if (record.encounterId != EncounterDef.encounterChapter3dotI) {
        record.consumedItemsForPlayer(player).forEach((itemName) {
          encounter?.logEvent('encounter_consume_item', parameters: {
            'item_name': itemName,
          });
    ...
    for (var milestone in record.campaignMilestones) {
      encounter?.logEvent('encounter_milestone', parameters: {
        'milestone': milestone,
      });
      _campaign.milestones.add(milestone);
    ...
    for (final entry in record.codexEntries.entries) {
      final number = entry.key;
      final title = entry.value;

      encounter?.logEvent('encounter_codex', parameters: {
        'codex_number': number.toString(),
        'codex_title': title,
      });
      _campaign.addCodexEntry(
          number: number, title: title, expansion: encounter?.expansion);
    }

    if (encounter != null) {
      Analytics.logEncounterComplete(encounter, record: record);
    }
  ...
  List<FigureDef> get adversaries {
    final expansions = _campaign.expansions;
    return campaignDefinition.adversaries.values
        .where((f) =>
            f.expansion == null ||
            expansions.contains(f.expansion?.toLowerCase()))
        .toList();
  }
  ...
  QuestDef? questForEncounter(String encounterId) {
    return _campaignDefinition?.quests.firstWhereOrNull((quest) {
      return quest.encounters.any((encounter) => encounter.id == encounterId);
    });
  }
}
...
extension CampaignConvenience on Campaign {
  bool get hasCompletedCoreCampaign {
    return completedQuests.any((element) => element.id == '9');
  }

  bool get isPlayingXulcCampaign {
    return expansions.contains(xulcExpansionKey) &&
        (skipCore || hasCompletedCoreCampaign);
  }
  ...
  bool hasStartedQuest(String questId) {
...
extension QuestDefinitionConvenience on QuestDef {
  ...
  bool get isChapter {
    return encounters.isEmpty || encounters.length == 1;
  }
  ...
  QuestStatus get status {
    ...
    if (isBlocked) {
      return isChapter ? QuestStatus.completed : QuestStatus.blocked;
    }
    if (isLocked) {
  ...
  bool get isCompleted {
    ...
    if (encounters.isEmpty || isChapter) {
      return false;
    }
...
extension EncounterDefinitionConvenience on EncounterDef {
  Future<void> logEvent(String name, {Map<String, String>? parameters}) async {
    await Analytics.logEncounterEvent(this, name, parameters: parameters);
  }
