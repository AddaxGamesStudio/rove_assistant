...
class EncounterState {
  ...
  @JsonKey(name: 'codices', defaultValue: {})
  Map<String, (int, int)> codicesTriggered = {};
  ...
  int countForCodex(String title) {
    return codicesTriggered.containsKey(title)
        ? codicesTriggered[title]!.$1
        : 0;
  }

  didApplyCodex(String key) {
    codicesTriggered[key] = (countForCodex(key) + 1, round);
  }
  ...
  int codexCount({required String title}) {
    return codicesTriggered[title]?.$1 ?? 0;
  }
  ...
  EtherField drawField() {
    switch (encounterId) {
      case EncounterDef.encounter4dot5:
        ...
        break;
      case EncounterDef.encounterIdot2:
      default:
        fieldsToDraw ??= [
          EtherField.wildfire,
          EtherField.everbloom,
  ...
  addCodexLink(
    ...
    final index = codexLinks.indexWhere((r) => r.$1 == number || r.$2 == title);
    if (index != -1) {
      codexLinks.removeAt(index);
    }
    codexLinks.add((number, title, trigger));
  }

  void removeCodexLinkByTitle(String title) {
    final index = codexLinks.indexWhere((r) => r.$2 == title);
    if (index != -1) {
      codexLinks.removeAt(index);
    }
  }
