...
import 'package:rove_assistant/model/encounter_executor.dart';
import 'package:rove_assistant/model/encounter_event.dart';
import 'package:rove_assistant/model/encounter_resolver.dart';
...
class EncounterModel extends ChangeNotifier {
  ...
  static forEncounter(EncounterDef encounter,
  ...
  Future<void> _onStartEncounter() async {
    ...
    _history.clear();
    _history.add(jsonEncode(encounterState.toJson()));
  }
  ...
  void setObjectiveAchieved(bool value) {
    final replaceWithMilestone = CampaignConfig.instance
        .replaceVictoryWithMilestone(encounterId: encounterDef.id);
    if (replaceWithMilestone != null &&
        !encounterState.milestones.contains(replaceWithMilestone)) {
      encounterState.milestones.add(replaceWithMilestone);
      _executor.onMilestone(replaceWithMilestone);
    } else {
  ...
  void spawnAdversary({required String name}) {
    encounterState.addPlacements(placements: [
      PlacementDef(name: name),
    ]);
    _onStateChanged(key: null);
  }
  ...
  void recordMilestone(String milestone, {required String title}) async {
    await _executor.recordMilestone(milestone, title: title);
    _onStateChanged(key: null);
  }
  ...
  void applyCodex(EncounterTrackerEventDef codex) {
    _executor.applyTrackerEvent(codex);
    _onStateChanged(key: null);
  }
