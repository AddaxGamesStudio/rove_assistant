...
import 'package:rove_assistant/model/encounter_model_extensions.dart';
...
class EncounterExecutor {
  ...
  Future<void> _showCodex(EncounterAction action) async {
    ...
    String? codexTitle = action.title;
    if (codexTitle == null) {
      final number = int.tryParse(action.value);
      if (number == null) {
        return;
      } else {
        await _showCodexByNumber(number);
        return;
      }
    }
    final key = 'codex:$codexTitle';
    if (state.countForCodex(key) > 0) {
      return;
    }
    state.didApplyCodex(key);
    final event = EncounterEvent.codexPrompt(
        model: model, title: codexTitle, body: action.body, page: action.value);
    // Remove codex link after creating the event to get its data.
    state.removeCodexLinkByTitle(codexTitle);
    events.addEvent(event);
  }

  Future<void> _showCodexByNumber(int number) async {
  ...
  void applyTrackerEvent(EncounterTrackerEventDef trackerEvent,
      {Figure? eventTarget, bool loot = false}) {
    final milestone = trackerEvent.recordMilestone;
    if (milestone != null) {
      recordMilestone(milestone, title: trackerEvent.title);
    }

    state.didApplyCodex(trackerEvent.title);
  }
